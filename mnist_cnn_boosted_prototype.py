...
while True:
	...
	for i in range(2000):
		...
		feature = (x_feature[0]).tolist()
		x_packet = []
		startIdx = 0
		endIdx = 100
		endOfArray = False
		print('Waiting on feature verification')
		while True:
			if(endOfArray):
				# if we have reached the end of array, transmit eoa
				clientsocket.send(pickle.dumps('eoa'))
				break
			if(len(feature) - startIdx < 100):
				x_packet = feature[startIdx:len(feature)]
				endOfArray = True
			else: 
				x_packet = feature[startIdx:endIdx]
			serialized_packet = pickle.dumps(x_packet)
			...
			try:
				...
				help_msg = True
			# if packet was no good, resend
			if(help_msg):
				print('trying to resend a bad packet')
				endIdx = startIdx
		...
		if(not retryFlg):
			...
			if(clientMessage == 'bf'):
				print('BAD')
  				...
  				msg = clientsocket.recv(1024)
  				print('waiting on classification message')
  				clientMessage = pickle.loads(msg)
  				if(clientMessage == 'bc'):
  					retryFlg = True
